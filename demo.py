...
def main():
    pokemons = json.load(open('examples/pogo-optimizer/data/pokemon.json'))
    parser = argparse.ArgumentParser()
    parser.add_argument("-u", "--username", help="PTC Username", required=True)
    parser.add_argument("-p", "--password", help="PTC Password", required=True)
    parser.add_argument("-l", "--location", help="Location", required=True)
    parser.add_argument("-d", "--debug", help="Debug Mode", action='store_true')
    parser.add_argument("-n", "--name", help="Pokemon Name", required=True)
...
#                    if pokemons[poke.PokedexNumber - 1]['Name'] == args.name:
#                        money.append(poke.PokedexNumber)
#                        print("Within one step of %s (%sm %s from you):" % (other, int(origin.get_distance(other).radians * 6366468.241830914), direction))
#                        print'(%s) %s ' % (poke.PokedexNumber, pokemons[poke.PokedexNumber - 1]['Name'])
#                    else:
#                        print("No %s within %sm %s of you..." % (args.name, int(origin.get_distance(other).radians * 6366468.241830914), direction))
        found = False
        print('')
        for poke in visible:
            if pokemons[poke.pokemon.PokemonId - 1]['Name'] == args.name:
                found = True
                ...
                print("No %s near you...expanding search..." % (args.name))
